...
fun contourSegment(
    linearContourA: List<Vector3>,
    linearContourB: List<Vector3>,
    frame0: Matrix44,
    frame1: Matrix44,
    writer: VertexWriter
) {
    for (i in linearContourA.indices) {
        val v0a = linearContourA[i]
        val v1a = linearContourA[(i + 1).mod(linearContourA.size)]

        val v0b = linearContourB[i]
        val v1b = linearContourB[(i + 1).mod(linearContourB.size)]

        val v00a = (frame0 * v0a.xyz1).xyz
        val v01a = (frame0 * v1a.xyz1).xyz

        val v10b = (frame1 * v0b.xyz1).xyz
        val v11b = (frame1 * v1b.xyz1).xyz
